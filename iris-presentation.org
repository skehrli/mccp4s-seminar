...
- \( \{ True \} \ \mathtt{newCounter}() \ \{ l. \ \boxed{\circ
  n}_{g}^{\gamma} \ast Inv(l, 0) \} \)
- \(
  \{\boxed{\circ n}_{g}^{\gamma} \ast Inv(l, n)\}\
  \texttt{incr l}\
  \{u. \ u \geq n \ast \boxed{\circ n}_{g}^{\gamma} \ast Inv(l, n+1)\}
  \)
- \(Inv(l,n) := \boxed{\exists m. \ l \mapsto m \land m \ge  n} \)
